...
####################**********************************************************######################################################***********************************************************
####################**********************************************************######################################################***********************************************************
metadata<-read.csv("/home/andrea/Documents/bioinformatica_condivisa/klebsiella_patric/PATRIC_genome.csv",stringsAsFactors = FALSE,header=TRUE,fill=TRUE,sep="\t")
...
rgl.bg(color = "grey")
plot3d(umeval_new_tmpSEL[w,1],anno,umeval_new_tmpSEL[w,2],col = C2[finalCLSEL[w,1]], radius = 4, size=8,xlab="",zlab="")
...
for(i in 2:length(uY)-1){
  ...
  plot3d(umeval_new_tmpSEL[w,1],anno,umeval_new_tmpSEL[w,2],col = C2[finalCLSEL[w,1]],add=TRUE,size=8)
  
  
  
  
}
rgl.postscript("plot3dx.pdf","pdf") 
...
gt <- which(finalCLSEL[,2]>.9)
finalCLSEL <- finalCLSEL[gt,]

years <- years[gt]
Uy <- unique(years)

Ty <- as.data.frame(table(years),stringsAsFactors = FALSE)
UC <- unique(finalCLSEL[,1])
time_abu <- matrix(0,nrow=length(UC),ncol=length(Uy))
colnames(time_abu) <- Ty$years
for(i in 1:nrow(time_abu)){
  
  w <- which(finalCLSEL==UC[i])
  yy <- as.data.frame(table(years[w]),stringsAsFactors = FALSE)
  
  for(n in 1:nrow(yy)){
    
    c<- which(Ty$years==yy$Var1[n])
    time_abu[i,c]<- 100 * yy$Freq[n] / Ty$Freq[c]
    
    
  }
  
  barplot(time_abu[i,],names=colnames(time_abu),main=paste("PC",UC[i],"-",sum(yy$Freq)))
